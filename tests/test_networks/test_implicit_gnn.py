...
import siml
from siml.networks.implicit_gnn import ImplicitFunction, ImplicitGNN
...
def test__train_implicit_gnn():
    ...
    loss = trainer.train()
    np.testing.assert_array_less(loss, 0.1)
...
def test__not_accepting_settings(settings: dict):
    ...
    block_settings = siml.setting.BlockSetting(**settings)
...
def test__sample_equiblium_equation():
    ...
    B = torch.tensor(
        np.array(list(range(n_point))).reshape(1, n_point), dtype=torch.float32
    )
    W = torch.eye(1, dtype=torch.float32)

    X_new, _, status, _ = ImplicitFunction._forward_iteration(
        W=W, X=X_init, A=adj, B=B, phi=lambda x: x
    )

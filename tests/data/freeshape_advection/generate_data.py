...
class DataGenerator:
    ...
    def _generate_one_data(self, i_data):
        ...
        fem_data = femio.generate_brick(
            ...
            x_length=self.edge_length*(1 + np.random.rand())*n_x_element,
            y_length=self.edge_length*(1 + np.random.rand())*n_y_element,
            z_length=self.edge_length*(1 + np.random.rand())*n_z_element)
        fem_data.nodes.data = self.deform_data(fem_data.nodes.data)
    ...
    def deform_data(self, x):
        raw_displacement = np.stack([
            self.generate_displacement(x, axis=0),
            self.generate_displacement(x, axis=1),
            self.generate_displacement(x, axis=2) * 0.,
        ], axis=1)
        x = x \
            + self.edge_length * 0.9 \
            / np.max(np.linalg.norm(raw_displacement, axis=1)) \
            * raw_displacement
        return x

    def generate_displacement(self, x, axis):
        x_max = np.max(x[:, axis])
        x_min = np.min(x[:, axis])
        l_x = x_max - x_min

        y_axis = (axis + 1) % 3
        y_max = np.max(x[:, y_axis])
        y_min = np.min(x[:, y_axis])
        l_y = y_max - y_min

        z_axis = (axis + 2) % 3
        z_max = np.max(x[:, z_axis])
        z_min = np.min(x[:, z_axis])
        l_z = z_max - z_min

        raw_displacement = np.sum([
            np.random.rand() * np.sin(
                (i_fourier + 1) * np.pi / l_x * x[:, axis])
            * (
                + np.random.rand() * np.sin(
                    (i_fourier + 1) * np.pi / l_y * x[:, y_axis])
                + np.random.rand() * np.sin(
                    (i_fourier + 1) * np.pi / l_z * x[:, z_axis])
            )
            for i_fourier in range(4)], axis=0)
    ...
    def transform_data(self, fem_data, target_dict_data):
        transformed_fem_data = femio.FEMData(
            nodes=fem_data.nodes, elements=fem_data.elements)
        rotation_matrix = stats.special_ortho_group.rvs(3)
        # rotation_matrix = np.eye(3)
        scale = (np.random.rand() + .1) * 3 / 1.1  # Varies [0.3, 3.0]

        transformed_fem_data.nodes.data = np.einsum(
            'kl,il->ik',
            rotation_matrix, transformed_fem_data.nodes.data) * scale
        target_dict_data['global_speed'] = np.einsum(
            'kl,ilf->ikf', rotation_matrix, target_dict_data['global_speed']) \
            * scale
        target_dict_data['cell_speed'] = np.einsum(
            'kl,ilf->ikf', rotation_matrix, target_dict_data['cell_speed']) \
            * scale
        return target_dict_data, transformed_fem_data
    ...
    def extract_feature(self, fem_data, target_dict_data):
        ...
        dict_data = {
            ...
            'facet_length': facet_area**.5,
            'inverse_facet_length': 1 / facet_area**.5,

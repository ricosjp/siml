...
class TestGroups(unittest.TestCase):
    ...
    def test_group_simple_restart_not_overwriting(self):
        main_setting = setting.MainSetting.read_settings_yaml(
            Path('tests/data/rotation_thermal_stress/group_simple.yml'))
        main_setting.trainer.gpu_id = GPU_ID
        tr = trainer.Trainer(main_setting)
        ...
        main_setting.trainer.restart_directory = \
            main_setting.trainer.output_directory
        main_setting.trainer.output_directory = \
            main_setting.trainer.output_directory.parent / "new_restart"
        tr = trainer.Trainer(main_setting)
        if tr.setting.trainer.output_directory.exists():
            shutil.rmtree(tr.setting.trainer.output_directory)
        loss = tr.train()
    ...
    def test_group_steady(self):
        main_setting = setting.MainSetting.read_settings_yaml(
            Path('tests/data/heat_steady/steady.yml'))
        main_setting.trainer.gpu_id = GPU_ID
        tr = trainer.Trainer(main_setting)
        if tr.setting.trainer.output_directory.exists():
            shutil.rmtree(tr.setting.trainer.output_directory)
